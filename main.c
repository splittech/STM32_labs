...
/*	
	...
	ВАЖНО:
		Так как данный проект работает в симуляторе, результат работы можно просмотреть в keil при помощи
		логического анализатора и значений переменных в режиме отладки;переменые, которые удобно отследить
		для понимания работы: mode, current_time, alarm_time.
		Также для имитации нажатия кнопок в keil использовалось окно Peripherals -> General purpose I/O -> GPIOA;
		в данном окне возможно изменять параметр pins при помощи чекбоксов и тем самым подавать сигнал на определнный пин,
		имитируя подачу сигнала от кнопки
*/
/*----------------------------------------------------------------------*/
